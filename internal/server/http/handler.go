...
func (ph *PreviewerHandler) fill(w http.ResponseWriter, r *http.Request) {
	...
	// Retrieve or download the image
...
func parseFillParams(urlPath string) (width, height int, imageURL string, err error) {
	...
	if err != nil {
		return 0, 0, "", fmt.Errorf("invalid width parameter: %v", err)
	}

	height, err = strconv.Atoi(parts[1])
	if err != nil {
		return 0, 0, "", fmt.Errorf("invalid height parameter: %v", err)
	}
	...
	imageURL = "https://" + path.Join(parts[2:]...)

	if _, err = url.ParseRequestURI(imageURL); err != nil {
		return 0, 0, "", fmt.Errorf("invalid image URL: %v", err)
	}
